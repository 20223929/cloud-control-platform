  ...
  <div class="app-container">
    ...
    >
      <template v-slot:toolbar_buttons>
        ...
        <el-button v-permission="['gasStationQuery:export']" type="primary" @click="exportExcel">导出</el-button>
        <el-button type="default" @click="handleReset">重置</el-button>
        <el-button @click="getData">刷新</el-button>
<!--        <el-button @click="showSearch = !showSearch">搜索</el-button>-->
      </template>
      <template v-slot:operation="{ row }">
        <el-button v-permission="['gasStationQuery:detail']" type="primary" @click="detail(row)" style="float: left">
          详情
        </el-button>
      </template>
    </vxe-grid>
    <detail ref="gasStationQueryDialog" @refreshData="getData"></detail>
  </div>
</template>

<script>
import permissionMixin from 'ecip-web/utils/permission-mixin'
import {getData} from '@/api/modules/querymanage/gasStationQueryApi'
import Detail from './components/detail'
import {download} from 'ecip-web/utils'

export default {
  name: 'ModulesQuerymanageGasStationQueryView',
  components: {Detail},
  ...
  data() {
    return {
      ...
      showSearch: true,
      loading: false,
      exportUrl: 'api/v1/querymanage/gasStationQuery/exportExcel',
      tableToolbar: {
        id: 'modules.querymanage.gasStationQueryView-toolbar',
        custom: true,
        slots: {
      ...
      columns: [
        ...
        {title: 'ETC卡号', field: 'etcCardId', minWidth: 180, align: 'center'},
        {title: '交易模式', field: 'modelTypeName', minWidth: 180, align: 'center'},
        {title: '加油量(升)', field: 'detailInfo.fuelVolume', tableField: 'fuel_volume', minWidth: 180, align: 'center'},
        {title: '交易金额(元)', field: 'tradeFee', tableField: 'trade_fee', minWidth: 180, align: 'center'},
        {title: '交易时间', field: 'tradeTime', tableField: 'trade_time', minWidth: 180, align: 'center', sortable: true},
        {title: '状态', field: 'statusName', tableField: 'status', minWidth: 180, align: 'center'},
  ...
  methods: {
    async getData() {
      ...
      this.tableData.forEach((item, index, array) => {
        item.tradeFee = (item.tradeFee / 100).toFixed(2)
        item.detailInfo.fuelVolume = item.detailInfo.fuelVolume.toFixed(3)
      })
    ...
    detail(row) {
      const record = row
      this.$refs.gasStationQueryDialog.init('view', record)
    },
    async exportExcel() {
      this.loading = true
