    ...
    >
      <el-form ref="form" v-loading="loading" class="form-full" :model="formItem" label-width="120px" label-suffix=" : ">
        ...
        <el-row :gutter="15">
          <el-col :span="12">
            <el-form-item
              label="服务类型，2-停车场；3-加油站；4-服务器；5-市政拓展；6-充电桩"
              prop="serviceType"
              :rules="[
                { required: true, message: '服务类型，2-停车场；3-加油站；4-服务器；5-市政拓展；6-充电桩不能为空', trigger: 'blur' }
              ]"
            >
              <el-input-number v-model="formItem.serviceType" type="text" placeholder="服务类型，2-停车场；3-加油站；4-服务器；5-市政拓展；6-充电桩"  />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
import { findByPrimaryKey, save, update } from '@/api/modules/report/agentReportApi'

export default {
  name: 'ModulesReportAgentReportForm',
  components: {},
  data() {
    return {
      visible: false,
  ...
  methods: {
    async init(method, record) {
      ...
      this.record = record
      if (method === 'add') {
        this.title = `新建代理商收益报表`
      } else if (method === 'edit') {
        this.title = '修改代理商收益报表'
      } else if (method === 'view') {
        this.title = '查看代理商收益报表'
      }
      this.visible = true
      this.loading = false
      await this.$nextTick()
