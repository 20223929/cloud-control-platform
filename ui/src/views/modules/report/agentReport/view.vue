  ...
  <div class="app-container">
    ...
    >
      <template v-slot:toolbar_buttons>
        ...
        <el-button v-permission="['agentReport:export']" type="primary" @click="exportExcel">导出</el-button>
...
import { getData, deleteByIds, getMerchantTreeData } from '@/api/modules/report/agentReportApi'
import { download } from 'ecip-web/utils'
import RejectDetail from "@/views/modules/settlementReject/view";

export default {
  name: 'ModulesReportAgentReportView',
  ...
  data() {
    return {
      ...
      importUrl: 'api/v1/report/agentReport/importExcel',
      tplUrl: 'api/v1/report/agentReport/excelTemplate',
      exportUrl: 'api/v1/report/agentReport/exportExcel',
      tableToolbar: {
        id: 'modules.report.agentReportView-toolbar',
        custom: true,
        slots: {
      ...
      columns: [
        ...
        { title: '代理商佣金费率', field: 'agentRate', tableField: 'agent_rate', minWidth: 180, sortable: false, align: 'center' },
        { title: '代理商佣金（元）', field: 'agentAmt', tableField: 'agent_amt', minWidth: 180, sortable: false, align: 'center' },
        { title: '中心服务费（元）', field: 'centerServiceAmt', tableField: 'center_service_amt', minWidth: 180, sortable: false, align: 'center' },
  ...
  methods: {
    async getData() {
      ...
      this.tableData.forEach((item,index,array) => {
        ...
        item.agentAmt = (item.agentAmt).toFixed(6)
        item.centerServiceAmt = (item.centerServiceAmt).toFixed(6)
      })
    ...
    add() {
      this.$refs.agentReportDialog.init('add')
    },
    edit(row) {
      const record = row || this.selections[0]
      this.$refs.agentReportDialog.init('edit', record)
    },
    deleteByIds(rowId) {
      ...
      this.$confirm(`确定删除${additionMsg}?`, '提示', {
        ...
        if (rowId) {
          ...
          deleteIds.push(...this.selections.map(row => row.sysId))
        }
    ...
    footerMethod({ columns, data }) {
      ...
      const footData = [
        columns.map((column, columnIndex) => {
          ...
          } else if (['serviceAmt', 'agentAmt', 'centerServiceAmt'].includes(column.property)) {
            return (XEUtils.sum(data, column.property)).toFixed(6)
          } else if (['serviceRate', 'agentRate'].includes(column.property)) {
            return '-'
