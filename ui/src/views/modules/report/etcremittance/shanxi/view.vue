  ...
  <div class="app-container">
    ...
    >
      <template v-slot:toolbar_buttons>
        ...
        <el-button v-permission="['etcRemittanceReport:export']" type="primary" @click="exportExcel">导出</el-button>
...
import {getData} from '@/api/modules/report/shanxi/etcRemittanceReportApi'
import {download} from 'ecip-web/utils'

export default {
  name: 'ModulesReportEtcRemittanceReportView',
  ...
  data() {
    return {
      ...
      showSearch: true,
      loading: false,
      exportUrl: 'api/v1/report/etcRemittanceReport/exportExcel',
      tableToolbar: {
        id: 'modules.report.etcRemittanceReportView-toolbar',
        custom: true,
        slots: {
      ...
      columns: [
        ...
        {title: '汇款日期', field: 'remittanceDay', tableField: 'remittance_day', minWidth: 180, align: 'center'},
        {title: '已汇款流水数', field: 'remittanceCount', tableField: 'remittance_count', minWidth: 180, align: 'center'},
        {title: '已汇款总金额', field: 'remittanceFee', tableField: 'remittance_fee', minWidth: 180, align: 'center'}
      ],
      transTime: '',
      queryParams: {
        timeScope: [],
  ...
  methods: {
    async getData() {
      ...
      this.tableData.forEach(item => {
        item.remittanceFee = (item.remittanceFee / 100).toFixed(2);
      })
    ...
    footerMethod({columns, data}) {
      ...
      const footData = [
        columns.map((column, columnIndex) => {
          ...
          if (['merchantGroupName', 'merchantName', 'siteName', 'tradeDay', 'remittanceDay'].includes(column.property)) {
            return []
          }
    ...
    spanMethod({row, _rowIndex, _columnIndex}) {
      if (row.merchantGroupName == '小计' && _columnIndex == 0) return {rowspan: 1, colspan: 5}
      else if (row.merchantGroupName == '小计' && (_columnIndex >= 1 && _columnIndex <= 4)) return {
        rowspan: 0,
        colspan: 0
      }
    },
    defaultScope() {
      var transTime = []
      var now = new Date();
      var before = 13 * 24 * 60 * 60 * 1000
      transTime.push(this.$moment(now - before).startOf('day').format('YYYY-MM-DD HH:mm:ss'))
      transTime.push(this.$moment(now).endOf('day').format('YYYY-MM-DD HH:mm:ss'))
