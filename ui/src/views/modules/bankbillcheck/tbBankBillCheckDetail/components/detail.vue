...
import { findByPrimaryKey, save, update } from '@/api/modules/bankbillcheck/tbBankBillCheckDetailApi'

export default {
  name: 'ModulesBankbillcheckTbBankBillCheckDetailForm',
  components: {},
  data() {
    return {
      visible: false,
  ...
  methods: {
    async init(method, record) {
      ...
      this.record = record
      if (method === 'add') {
        this.title = `新建银行对账明细表`
      } else if (method === 'edit') {
        this.title = '修改银行对账明细表'
      } else if (method === 'view') {
        this.title = '查看银行对账明细表'
      }
      this.visible = true
      this.loading = false
      await this.$nextTick()
